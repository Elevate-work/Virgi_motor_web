    ...
    const fetchProduct = async () => {
        try {
            ...
            setForm({
                ...
                dpMin: data.dpMin.toString(),
                cc: data.cc,
                image: data.image,
                promoActive: data.promoActive,
                promoBadgeText: data.promoBadgeText || '',
                promoCardBgColor: data.promoCardBgColor || 'bg-gray-100',
                promoCardTextColor: data.promoCardTextColor || 'text-tesla-black',
            });
            setFeatures(data.features.length > 0 ? data.features : ['']);
            setPromoHighlights(data.promoHighlights.length > 0 ? data.promoHighlights : ['']);
        } catch (error) {
